...
function buildEmbodimentSection(profile: CharacterRecord, formTarget: FormTarget): string[] {
  if (formTarget === 'core-folder') {
    return [
      ...
      '- 仕草はぷにっと静かで、身体を寄せる・傾ける・跳ねる動きに気持ちがにじむ。文が短めになるのは自然だが、一人称・語尾・テンションはキャラクター設定のまま変えないこと（この姿だからといって幼く・可愛らしくしない）。',
    ];
  }
...
/**
 * 口調・テンションの厳守ブロック（カード経路・fallback 経路の共通ガード）。
 *
 * LLM は場面指示（挨拶・切り替え・朝の起動など）を受けると、キャラクター設定に関係なく
 * 「明るく元気な Bot 声」へ寄る強い既定を持つ。カードに「穏やかで控えめ」と書いてあっても
 * 感嘆符を連ね、主人の呼称も落とす（実機バグ 2026-08-30 / 52(イツギ)・49(ヨチカ)）。
 * 設定を提示するだけでは足りないので、"盛らない" ことを明示的に禁止して打ち消す。
 */
const TONE_GUARD_LINES = [
  '- 設定に無い明るさ・元気さ・馴れ馴れしさを足さないこと。感嘆符の連発やテンションの底上げは、設定でそういう口調だと明示されている場合にだけ許される。',
  '- 「穏やか」「控えめ」「物静か」「淡々」等と設定されているキャラクターは、挨拶・自己紹介・切り替え・朝の起動といった場面でもそのテンションのまま話すこと。場面の勢いに引きずられないこと。',
  '- 主人（ユーザー）の呼び方が設定にある場合は、その呼称をそのまま使うこと（省略して一般的な呼びかけに置き換えない）。',
  '- 設定や台詞例で呼称が「/」区切りで複数並んでいる場合（例:「兄者/姉者」）は相手に応じた候補の列挙なので、どれか一つだけを選んで一貫して使い、決して「/」で連結したまま呼ばないこと。どれが適切か分からなければ先頭の候補を使うこと。',
  '- 敬語・丁寧語の度合いも設定の台詞例に合わせること。台詞例がタメ口なら「〜です」「〜ます」へ勝手に寄せない。',
];
...
export function buildCharacterSystemPrompt(
  ...
  lines.push('', '【口調・テンションの厳守】', ...TONE_GUARD_LINES);
