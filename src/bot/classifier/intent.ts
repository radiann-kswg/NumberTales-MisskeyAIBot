...
const TASK_LIST_PATTERNS: RegExp[] = [
  // 「タスクの一覧」「タスク一覧を教えて」「タスクを見せて」等、タスク語と要求語の間に助詞
  // （の/を）が挟まる言い回しも拾う。旧 /タスク(?:を?見せて|一覧|確認|リスト)/ は助詞「の」を
  // 挟むと非マッチで「タスクの一覧は確認できる？」を雑談へ取りこぼしていた（実機バグ 2026-07-21）。
  /タスク(?:の|を)?(?:見せて|一覧|リスト|確認|教えて)/,
  /残り(?:のタスク|やること|どれくらい)/,
  /進捗(?:は?|を教えて|確認)/,
...
const TASK_ADD_PATTERNS: RegExp[] = [
  // 「タスク「〇〇」を優先度「中」…で追加して」のように、タスク語と登録動詞の間に題名・
  // パラメータ（鉤括弧付き）が挟まる明示的な登録依頼を拾う。旧パターンはリテラルの
  // 「タスクに追加」「登録して」等しか拾えず、動詞が「追加して」で題名が間に入るこの言い回しを
  // 雑談へ取りこぼしていた（実機バグ 2026-07-21）。動詞は「追加/登録」＋「して/お願い」等に
  // 限定し、「追加した(=完了報告)」「追加方法(=質問)」には反応しないようにしている。
  // 「追加したい」（願望形）も登録依頼として拾う（実機バグ 2026-09-16: 「「〇〇」のタスクを
  // 追加したい」が雑談へ落ち、LLM が登録のフリだけ返して DB 未書き込みだった）。
  /タスク[\s\S]*(?:追加|登録)(?:して|しといて|しておいて|してほしい|してくれ|したい|お願い|よろしく)/,
