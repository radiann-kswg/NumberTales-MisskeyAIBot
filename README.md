...
| 「ヒット＆ブロウやって」「6桁で重複ありのヒットアンドブロウ」 | **ヒット＆ブロウ** | Bot が 2〜8 桁（デフォルト4桁）の数字を設定（「重複あり」指定で桁の重複も許容）・当てる数当てゲーム（回答ログは色分けした数字タイルで CW 記録: 赤=ヒット/青=ブロウ/白=非該当） |
...
> | ヒット＆ブロウ | `sv_kougyoku_0`〜`9`（赤=ヒット）・`sv_seiyuu_0`〜`9`（青=ブロウ）・`sv_hakuji_0`〜`9`（白=非該当）の色分け数字タイル |
...
| 「タスク一覧見せて」「進捗はどのくらい？」 | 登録済みタスクと重み付き進捗％を表示 |
| 「設計書終わった」「3番のタスク完了」 | タスクを完了にする（内容一致 or 番号指定。複数候補時は番号で再指定を促す） |
...
- 配信は5分間隔のスケジューラーがチェックし（1回あたり最大5件の安全上限つき）、`visibility: home` で
  本人宛てメンション付きノートとして送信。期日超過も別途1回だけ通知
- 進捗％は優先度×難易度の重み付き計算（易しい低優先タスクを大量にこなしても伸びにくい設計）
...
src/
  ...
  bot/
    ...
    scheduler/
      ...
      task-scheduler.ts       # F-12 タスク通知配信スケジューラー（5分間隔・安全上限5件）
  config/                     # 環境変数・定数
  features/
    ...
    task/index.ts             # F-12: LLM日時抽出・進捗%計算・一覧整形・対象特定
