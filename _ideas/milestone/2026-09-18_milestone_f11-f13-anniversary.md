...
- **月曜の記念日投稿が落ちうる**: 月曜 7 時台は就任挨拶で `return` するため、6 時台の tick までに
  実施できなかった場合、就任挨拶のクールダウン（1〜2 時間）が朝スロットの終了（8 時）を越えて
  その日の記念日投稿が出ない。年に数回の取りこぼしを許容し、専用スロットは設けていない
  （頻発するようなら記念日を独立スロットへ切り出す）
- **春分の日は 3/21 固定**（年によって 3/20 になる年はズレる）
- キャラクターの記念日は `AnivDay` を持つ released キャラのみが対象。DB 側に記念日が未設定の
  キャラは祝われない
