# 昇進済みアイデア一覧 — ナンバーテールズ Misskey AI Bot

このフォルダは、[`_ideas/future-plan/`](../) 配下のアイデアメモのうち
**`_ideas/milestone/` に正式な実装仕様（milestone ドキュメント）として昇進したもの**を
棚卸し・格納するためのフォルダです。

昇進済みのアイデアは、ここに置かれた元アイデアファイルではなく
**対応する milestone ドキュメントが正式仕様**になります。詳細はリンク先を参照してください。
...
- [`F-06_stage-d-minigames.md`](../F-06_stage-d-minigames.md): D3-1・D3-2・D3-2c・D3-6・D3-7 は milestone ドキュメントへ
  昇進済み（各 milestone ドキュメント参照）。D3-8（ポーカー・麻雀の手札/手牌交換、2026-07-05）・D3-5
  （キャラ番号ルーレット、2026-07-08）は実装済みだが、独立した milestone ドキュメントは作成せず本ファイル上で
  ステータス更新のみ行ったため未昇進扱い。D3-4a・D3-4b は実装前に設計を詰めている段階のため本体はここに
  移動していない。
...
進行中・未着手のアイデアは [`_ideas/future-plan/`](../) を参照してください。
